      ...
      <div className="grid md:grid-cols-3 gap-6 pt-4">
        <div className="md:col-span-1 space-y-6">
          <Card className="p-6">
            ...
            {isEditing ? (
              <textarea
                className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white text-sm focus:ring-2 focus:ring-nexus-500 outline-none"
                rows={4}
                defaultValue="Passionate AI Engineer focused on AgriTech solutions."
              />
            ) : (
              <p className="text-sm text-slate-400">
                Passionate AI Engineer focused on AgriTech solutions. Experienced in Python,
            ...
            <div className="mt-6 space-y-3">
              ...
              <div className="flex items-center gap-2 text-sm text-slate-300">
                <Github size={16} className="text-slate-500" /> github.com/
                {user.name.split(' ')[0].toLowerCase()}
              </div>
              <div className="flex items-center gap-2 text-sm text-slate-300">
                <Linkedin size={16} className="text-slate-500" /> linkedin.com/in/
                {user.name.split(' ')[0].toLowerCase()}
              </div>
            </div>
          </Card>
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <Card className="p-6">
            ...
            <p className="text-sm text-slate-400 mb-4">
              Add skills to be matched with Industry Partners (e.g., Python, Computer Vision,
              Fintech).
            </p>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {skills.map((skill) => (
                <span
                  key={skill}
                  className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-nexus-900/40 text-nexus-300 border border-nexus-700/50"
                >
                  {skill}
                  {isEditing && (
                    <button
